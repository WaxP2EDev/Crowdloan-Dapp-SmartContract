...
void crowdloan::stakecheck(const name &from, const name &to,
...
void crowdloan::logstake(const name account, const time_point_sec unstakeTime,
...
void crowdloan::inlineunstak(const uint64_t id) {
...
void crowdloan::adminunstake(const uint64_t id) {
  require_auth(get_self());
  inlineunstak(id);
}

void crowdloan::admintransfr(){

  require_auth(get_self());
  name transferTo = name("crowdloanpool2");  //Change A/C 
...
void crowdloan::logunstake(const asset unstakedcpu, const name account) {
  require_auth(get_self());
}

void crowdloan::testreset() {
  require_auth(get_self());
...
int crowdloan::rfinder(vector<pair> cpu_multipliers, int days) {
